...
exports.getJobs = async (req, res) => {
  try {
      ...
      if (trimmedSearch) {
        ...
        if (trimmedSearch.split(" ").length > 3) {
          try {
            ...
            if (aiFilters.role)
              query.title = {
                $regex: new RegExp(escapeRegex(aiFilters.role), "i"),
              };
            if (aiFilters.location)
              query.location = {
                $regex: new RegExp(escapeRegex(aiFilters.location), "i"),
              };
    ...
    if (industry && industry !== "undefined") {
      query.industry = { $regex: new RegExp(escapeRegex(industry), "i") };
    }
    ...
    try {
      await sendEmail(
        ...
        <div style="font-family: sans-serif; padding: 20px; color: #333;">
          ...
          <p style="font-size: 12px; color: #999;">This is an automated notification from SmartHire.</p>
        </div>
        `,
      );
    } catch (emailErr) {
